...
class MiniRouter extends HTMLElement {
    ...
    __setListeners() {
        if (!window)
            console.warn('window Object missing');
    ...
    __handleRouting(e) {
        ...
        for (var c = this.children.length, i = 0; i < c; i++) {
            ...
            if (path && path === route) {
                ...
                let j, regex = pattern.split('/').map((i) => {
                    // Pattern without regex
                    if (/^:[a-zA-Z0-9]+(@[a-zA-Z0-9]+)?$/.test(i))
                        return '[^\/]+';
                ...
                if (regex && (new RegExp(regex)).test(route)) {
                    ...
                    variables = pattern.split('/').reduce((acc, item, index) => {
                        ...
                        if (key != null) {
                            //@ts-expect-error
                            if (key[1])
                                acc[key[1]] = routeparts[index];
                            if (key[5])
                                child.setAttribute(key[5], routeparts[index]);
                        }
                        return acc;
                    }, {});
                }
        ...
        if (!matched) {
            let fallback = this.querySelector('[data-fallback]');
            if (fallback && fallback.parentNode === this) {
                fallback.style.display = 'initial';
            }
        }
    }
    __getProp(element, prop) {
